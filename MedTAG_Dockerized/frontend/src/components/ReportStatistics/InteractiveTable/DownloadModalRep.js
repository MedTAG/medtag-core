...
function DownloadModalRep(props) {
    ...
    const [selectedModeAll,SetselectedModeAll] = useState('')
    const [selectedModeMajor,SetselectedModeMajor] = useState('')
    ...
    useEffect(()=>{
        if(selectedActMajor !== 'none'){
            ...
            axios.post("get_users_list", {action:selectedActMajor,reports:ids})
                .then(response => {
                    SetUsersList(response.data)
                })
                .catch(error=>{
    ...
    function downloadAllReports(){
        if(selectedActAll !== 'none' && selectedModeAll !== ''  && selectedFormatAll !== '') {
    ...
    function downloadMajorityReports(){
        if(selectedActMajor !== 'none' && selectedModeMajor !== '' && ChosenUsers.length > 1 && selectedFormatMajor !== '') {
            ...
            <Row>
                <Col  md={6}>
                    <div className='modalContainer' >
                        <h4>Download all the ground-truths created for the selected reports</h4>
                        {MissingParamsAll === true && <div>
                            <h6>Please, select a file format, an annotation type and annotation mode.</h6>
                        </div>}

                        <div className='sel_class'>Select an annotation type</div>
                        <div className='sel_class'><Form.Control value={selectedActAll} ref ={act} className='selection all_sel'  as="select" onChange={(e)=>{act.current = e.target.value;SetMissingParamsAll(false);SetselectedActAll(e.target.value);SetselectedFormatAll('');SetselectedModeAll('')}}>
                        ...
                        <div className='sel_class'><Form.Control value={selectedModeAll} className='selection all_sel' as="select" defaultValue="choose the annotation mode..." onChange={(e)=>{SetMissingParamsAll(false);SetselectedModeAll(e.target.value);mode.current = e.target.value}}>
                            ...
                            {(((EXAPresenceLabels === true && EXAPresenceConcepts === true) || (EXAPresenceLabels === true && selectedActAll === 'labels') || (EXAPresenceConcepts === true && selectedActAll !== 'none' && selectedActMajor !== 'labels')) && AutoPresence === true )&& <option value = 'both'>Manual & Automatic</option>}
                ...
                <Col  md={6}>
                    <div className='modalContainer'>
                        ...
                        <div className='sel_class'>Select an annotation type</div>
                        <div className='sel_class'><Form.Control value={selectedActMajor} as="select" defaultValue="Select an action..." onChange={(e)=>{SetMissingParamsMajor(false);SetselectedActMajor(e.target.value);SetselectedModeMajor('');SetselectedFormatMajor('');act.current = e.target.value;console.log('act',act.current)}}>
                            <option value = 'none'>Select an annotation type...</option>
                            <option value = 'labels'>Labels</option>
                            <option value = 'mentions'>Mentions</option>
                            <option value = 'concepts'>Concepts</option>
                        ...
                        <div className='sel_class'><Form.Control value = {selectedModeMajor} className='selection maj_sel' as="select" defaultValue="choose the annotation mode..." onChange={(e)=>{SetMissingParamsMajor(false);SetselectedModeMajor(e.target.value);mode.current = e.target.value}}>
                            ...
                            <option value = 'Human'>Manual</option>
                            {AutoPresence === true && <option value = 'Robot'>Automatic</option>}
                            {(((EXAPresenceLabels === true && EXAPresenceConcepts === true) || (EXAPresenceLabels === true && selectedActMajor === 'labels') || (EXAPresenceConcepts === true && selectedActMajor !== 'none' && selectedActMajor !== 'labels')) && AutoPresence === true )&& <option value = 'both'>Manual & Automatic</option>}
