...
function MajorityVoteModal(props) {
    ...
    const { index,username,loadingMentions,color,tokens,selectedLanguage,selectedUse,selectedInstitute,fields,mentionsList,loadingColors,showannotations,finalcount,fieldsToAnn,reached,orderVar, errorSnack,reports, reportString, insertionTimes } = useContext(AppContext);
    ...
    useEffect(()=>{
        ...
        axios.get("get_users_list", {params: {id_report:props.id_report,language:props.language}})
            .then(response => {
        ...
        axios.get("get_presence_robot_user", {params: {id_report:props.id_report,language:props.language}})
            .then(response => {
    ...
    useEffect(()=>{
        if(selectedAct !== 'none'){
            axios.get("get_users_list", {params: {action:selectedAct,id_report:props.id_report,language:props.language}})
                .then(response => {
                    SetUsersList(response.data)
                })
                .catch(error=>{
    ...
    function createGT(e){
        e.preventDefault()
        axios.post('create_majority_vote_groundtruth',{action:selectedAct,mode:selectedMode,users:ChosenUsers,id_report:props.id_report,language:props.language})
            .then(response=>{
    ...
    return (
        <div className='container-fluid'>
            ...
            {(newInd !== false  && ReportString !== '' && (Fields.length > 0 || FieldsToAnn.length > 0) && Reports.length > 0) ? <Row>
                ...
                <Col  md={6}>
                    {(UsersList.length === 0 && selectedAct === 'none') ? <div><b>This report has not been annotated yet.</b></div> :
                        <div>
                            {(MajorGT === false ) ? <div className='modalContainer'>
                                ...
                                <hr/>
                                {selectedAct !== 'none' && UsersList.length > 0 && <div><div style={{'margin-bottom':'1%','margin-top':'1%'}}>Select an Annotation Mode</div>
                                ...
                                {selectedAct !== 'none' && UsersList.length === 0 && <div>No user annotated <b>{selectedAct}</b> annotation type</div>}
                                {selectedAct !== 'none' && selectedMode !== '' && (UsersList.length > 0 || ChosenUsers.length > 0 || Username === 'Test') &&<div style={{'padding-left':'1%','padding-right':'1%'}}>
                                    ...
                                    {ChosenUsers.length > 0 && <div>
                                        ...
                                        <Button style={{marginBottom:'2%'}} disabled={ChosenUsers.length === 0 || selectedAct === 'none' || selectedMode === '' } onClick={(e)=>createGT(e)} variant='primary'>Confirm</Button>
                                    </div>}
                                    <hr/>
                                </div>}
