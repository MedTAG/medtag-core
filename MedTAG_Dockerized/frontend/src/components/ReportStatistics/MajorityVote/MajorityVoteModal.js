...
function MajorityVoteModal(props) {
    ...
    useEffect(()=>{
        if(selectedAct !== 'none'){
            axios.get("http://0.0.0.0:8000/get_users_list", {params: {action:selectedAct,id_report:props.id_report,language:props.language}})
                .then(response => {
                    SetUsersList(response.data)
                })
                .catch(error=>{
    ...
    function createGT(e){
        e.preventDefault()
        axios.post('http://0.0.0.0:8000/create_majority_vote_groundtruth',{action:selectedAct,mode:selectedMode,users:ChosenUsers,id_report:props.id_report,language:props.language})
            .then(response=>{
