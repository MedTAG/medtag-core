...
export default function TableToShow(props) {
    ...
    const [ColumnsNew, SetColumnsNew] = useState((props.columns).filter(c=>(c.name==='id_report' || c.name==='annotations'||c.name==='usecase'||c.name==='language'||c.name === 'batch' || c.name==='institute'||c.name==='')))
    ...
    useEffect(()=>{
        if(IDToStats){
            axios.get('annotation_all_stats',{params:{report:IDToStats['id_report'],language:IDToStats['language']}})
    ...
    function handleDeleteSingle(e,row){
        setSelection([])
        axios.post('delete_reports',{report_list:[row]}).then(response => {
    ...
    const handleDelete = ()=>{
        ...
        axios.post('delete_reports',{report_list:selected_rows}).then(response=>{
            console.log('new_rows',arr)
            console.log('new_rows',selection)
            setSelection([])
        ...
        <div>
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>Annotations stats REPORT {IDToStats['id_report']}</Modal.Title>
                </Modal.Header>
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title>Majority vote ground-truth REPORT {IDtoMajority['id_report']}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body className='body_anno'>
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>Report {IDtoText['id_report']}</Modal.Title>
                </Modal.Header>
            ...
            >
                ...
                <Modal.Body>
                    <div>
                        ...
                        You are going to delete the following report: {IDToRemove['id_report']}
                        <hr/>
                    </div>
