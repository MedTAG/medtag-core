...
function DownloadGT(props){
    const { showDownload,action,reportString,report_type,language,usecase,updateMenu,usecaseList,languageList,instituteList } = useContext(AppContext);
    ...
    function onSave(e,token){
        ...
        if((Format === '' || Act === '') && token !== 'all' ){
            ...
            axios.get('http://0.0.0.0:8000/get_gt_list',{params:{reptype:ReportType,token:token,action:Act,inst:Ins,lang:Lang,use:Use}})
                .then(response => {
