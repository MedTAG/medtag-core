...
function SelectMenu(props){
    ...
    function handleChangeInstitute(e){
        ...
        var opt_batches = []
        if(i === 'pubmed'){
            axios.get('get_PUBMED_batch_list', {params: {usecase: UseCase}}).then(response => {
                SetBatchList(response.data['batch_list']);
            ...
            axios.get('get_batch_list', {params: {usecase: UseCase}}).then(response => {
                SetBatchList(response.data['batch_list']);
                opt_batches = response.data['batch_list']
                var b = 1
                if (opt_batches.length > 0){
    ...
    function handleChangeUseCase(e){
        ...
        if(ReportType === 'pubmed'){
            axios.get('get_PUBMED_batch_list', {params: {usecase: e.target.value}}).then(response => {
                SetBatchList(response.data['batch_list']);
                opt_batches = response.data['batch_list']
                var b = 1
                if (opt_batches.length > 0){
            ...
            axios.get('get_batch_list', {params: {usecase: e.target.value}}).then(response => {
                SetBatchList(response.data['batch_list']);
                opt_batches = response.data['batch_list']
                var b = 1
                if (opt_batches.length > 0){
