...
function MyStats() {
    ...
    useEffect(()=>{
        ...
        axios.get('http://0.0.0.0:8000/get_presence_robot_user',{params:{username:username}}).then(function(response){
    ...
    useEffect(()=>{
        if(ChosenStats !== '' && SelectedLang !== '' && SelectedInstitute !== '' && SelectedBatch !== ''){
            ...
            axios.get("http://0.0.0.0:8000/get_stats_array_per_usecase",{params:{mode:ChosenStats,language:SelectedLang,institute:SelectedInstitute,batch:SelectedBatch}}).then(response => {
