...
function ChangeMemberGT(props){
        ...
        if(MakeReq){
            ...
            if(Action === 'labels'){
                ...
                axios.get("http://0.0.0.0:8000/annotationlabel/user_labels", {params: {language:Language,ns_id:ns_id,username:username_to_call,report_id: Reports[Index].id_report.toString()}}).then(response => {
                    ...
                    SetLoadingChangeGT(false);SetMakeReq(false)
                })
            }
                ...
                axios.get("http://0.0.0.0:8000/contains", {params: {language:Language,ns_id:ns_id,username:username_to_call,report_id: Reports[Index].id_report.toString()}}).then(response => {setSelectedConcepts(response.data);SetLoadingChangeGT(false)})
                SetMakeReq(false)
            }
        }
    ...
    useEffect(()=>{
        ...
        if(ChangeButton && Fields.length > 0 && MakeReq){ // Adedd 3092021
