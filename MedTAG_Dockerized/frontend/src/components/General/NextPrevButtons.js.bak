...
function NextPrevButtons(props){
        ...
        if(ShowAutoAnn === false && ShowMemberGt === false){
            ...
            if(LoadingReportList === false && LoadingReport === false){
                ...
                if (token.startsWith('mentions') && LoadingMentions === false && LoadingMentionsColor === false) {
                    ...
                    // }else if (token.startsWith('annotation') && LoadingLabels === false ) {
    ...
    return(
        ...
        <span  className='two_buttons_div_rep'>
                        ...
                        <Button id='but_dx' size='sm' disabled={LoadingReport || Reports.length === 1} className="btn prevbtn" type="submit" onClick={(e)=>submit(e,TokenNextPrev[1])} name = "prev"  variant="info"><FontAwesomeIcon icon={faChevronLeft} /></Button>&nbsp;&nbsp;
            ...
            <Button id='but_sx' size='sm' className="btn nextbtn" disabled={LoadingReport || Reports.length === 1} type="submit" onClick={(e)=>submit(e,TokenNextPrev[0])}  name = "next"  variant="info"><FontAwesomeIcon icon={faChevronRight} /></Button>
            </span>
