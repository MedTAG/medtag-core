...
function InfoAboutConfiguration() {
    ...
    function onSaveTemplate(e,token){
        e.preventDefault()
        axios.get('download_templates', {params:{token:token}})
            .then(function (response) {
                if(token === 'reports'){
    ...
    useEffect(()=>{
        console.log('ALL!!!')
        axios.get('get_gt_list',{params:{token:'all'}}).then(response => SetGroundTruthList(response.data['ground_truths'])).catch(error =>console.log(error))
    },[])
    ...
    function onSaveAll(token){
        ...
        if(token === 'save' && GroundTruthList>0){
            ...
            axios.get('download_all_ground_truths')
                .then(function (response) {
