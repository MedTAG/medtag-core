...
function Prova_BaseIndex() {
    ...
    useEffect(()=>{
        ...
        SetLoadingReport(false)
        if(Reports.length>0 &&  MakeReq === false && Index !== false) {
    ...
    return (
        <div className="App">
            ...
            {LoadingReportList  ? <div className='spinnerDiv'><Spinner animation="border" role="status"/></div> :
                            ...
                            <Col md={5}>
                                ...
                                {Action === 'concept-mention' && <div className='first_container_right'>
                                    {(LoadingAssociations || LoadingReport || LoadingChangeGT) ? <Spinner animation="border" role="status"/> :
                                        <LinkedContext.Provider value={{mountForm:[MountForm,SetMountForm], enable_select: [Enable, SetEnable], conceptchosen:[ConceptChosen, SetConceptChosen],  show:[Show,SetShow]}}>
