...
def en_sanitize_record(record,
                       use_case):  # @smarchesin TODO: define sanitize use-case functions that read replacements from file
    """
    ...
    if record:
        if use_case == 'colon':
            ...
            record = record.replace('hyperplasia',
                                    'hyperplastic')  # MarianMT translates 'iperplastico' as 'hyperplasia' instead of 'hyperplastic'
...
def radboud_colon_labels2binary(report_labels):
    ...
    for rid, rlabels in report_labels.items():
        ...
        # update binary labels w/ 1 in case of label presence
        if rlabels['labels']['cancer'] == 1 or rlabels['labels']['lgd'] == 1 or rlabels['labels'][
            'hgd'] == 1:  # update cancer_or_adenoma label
