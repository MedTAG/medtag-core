...
class RDFProc(object):
    ...
    def __init__(self):  # @smarchesin TODO: add Test outcome to predicate2literal list (?why?)
        ...
        self.gender = {'M': 'http://purl.obolibrary.org/obo/NCIT_C46109',
                       'F': 'http://purl.obolibrary.org/obo/NCIT_C46110'}
        self.age_set = {
    ...
    @staticmethod
    def associate_polyp2dysplasia(outcomes, mask_outcomes,
                                  debug=False):  # @smarchesin TODO: needs to be part of a dict of functions that are use-case dependants
        """
                ...
                if out[0] == '0':  # outcome is a dysplasia mention - skip it
                    ...
                    if ix + 1 < len(mask_outcomes):  # ix != last
                        if mask_outcomes[ix + 1][0] == '0':  # succeeding outcome is a dysplasia mention
                            dysplasia_outcomes = [outcomes[ix + 1 + oix][0] for oix in
                                                  range(len(mask_outcomes[ix + 1]))]
                            pairs.append([outcomes[ix][0]] + dysplasia_outcomes)
    ...
    def aoec_create_graph(self, rid, report_data, report_concepts, onto_proc, use_case, debug=False):
        ...
        if report_data['diagnosis_nlp']:  # textual diagnosis is present within report data
            rdf_graph.append(
                (report, self.namespace['exa'] + 'ontology/#hasDiagnosisText', report_data['diagnosis_nlp']))
        ...
        if 'age' in report_data:  # age data is present within report_data
            age = report_data['age']
        elif report_data['birth_date'] and report_data[
            'visit_date']:  # birth date and visit date are present - compute age
        ...
        if report_data['gender']:  # gender data is present within report_data
            rdf_graph.append(
                (patient, self.namespace['exa'] + 'ontology/#hasGender', self.gender[report_data['gender']]))
        ...
        report_outcomes = [
            (diagnosis[0], onto_proc.get_higher_concept(iri1=diagnosis[0], iri2=ontology_outcome)) for diagnosis in
            report_concepts['Diagnosis']]
        if use_case == 'colon':
        ...
        for pair in paired_outcomes:  # @smarchesin TODO: handle negative results -- i.e., when len(paired_outcomes) == 0
            ...
            if use_case == 'cervix':
                if pair[0] == ontology_hpv:  # outcome is hpv infection
                    rdf_graph.append(
                        (resource_outcome, self.namespace['exa'] + 'ontology/#detectedHumanPapillomaVirus', pair[0]))
            ...
            if use_case == 'colon' and len(pair) > 1:  # target outcome has associated dysplasia
                ...
                for dysplasia_outcome in pair[1:]:
                    ...
                    rdf_graph.append(
                        (resource_outcome, self.namespace['exa'] + 'ontology/#hasDysplasia', dysplasia_outcome))
            ...
            for ix, procedure in enumerate(
                    report_procedures):  # @smarchesin TODO: correct? we might link the same procedure to multiple outcomes
                intervention_struct = {}
                ...
                rdf_graph.append(
                    (resource_outcome, self.namespace['exa'] + 'ontology/#hasIntervention', resource_procedure))
    ...
    def radboud_create_graph(self, rid, report_data, report_concepts, onto_proc, use_case, debug=False):
        ...
        if 'slide_ids' in report_data:
            ...
            for slide_id in report_data['slide_ids']:
                ...
                rdf_graph.append(
                    (report + '/slide/' + slide_id, self.namespace['exa'] + 'ontology/#hasSlideId', slide_id))
