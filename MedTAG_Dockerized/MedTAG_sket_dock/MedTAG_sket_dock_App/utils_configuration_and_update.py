...
# def update_concepts(cursor,disease,usecase,load_concepts):
#
#     """Add to the database the EXAMODE concepts"""
#
#     workpath = os.path.dirname(os.path.abspath(__file__))  # Returns the Path your .py file is in
#     ontology_path = os.path.join(workpath, 'automatic_annotation\\ontology\\examode.owl')
#     ontology = owlready2.get_ontology(ontology_path).load()
#     sparql = "PREFIX exa: <https://w3id.org/examode/ontology#>\
#             PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\
#             select ?iri ?iri_label ?semantic_area ?semantic_area_label where { \
#             ?iri rdfs:label ?iri_label ; \
#             exa:AssociatedDisease ?disease.\
...
#     concept_has_uc = []
#     conc = []
#     description = {'provenance': 'EXAMODE', 'insertion_author': 'robot'}
#
...
#                         j_c['insertion_author'] = 'robot'
#                         if usecase in load_concepts:
...
def configure_concepts(cursor,load_concepts):
        ...
        with open(os.path.join(workpath, 'automatic_annotation/db_examode_data/examode_db_population.json'),
            ...
            for el in usecases:
                ...
                for label in data['labels'][el]:
                    seq = seq + 1
                    ar_tup_label.append((label, seq, el,))
            cursor.executemany("INSERT INTO use_case VALUES (%s)", ar_tup)
...
# def update_db_for_automatic_annotation(cursor,load_concepts):
#
#     """This method updated the db whether the automatic annotation is required: it inserts the labels and the concepts."""
#
#     workpath = os.path.dirname(os.path.abspath(__file__))  # Returns the Path your .py file is in
#     with open(os.path.join(workpath, 'automatic_annotation/db_examode_data/examode_db_population.json'),
#               'r') as outfile:
...
#             ar_tup.append((el,))
#             for label in data['labels'][el]:
#                 ar_tup_label.append((label, seq + 1, el,))
#                 seq = seq + 1
#         cursor.executemany("INSERT INTO use_case VALUES (%s)", ar_tup)
#         cursor.executemany("INSERT INTO annotation_label (label,seq_number,name) VALUES (%s,%s,%s)", ar_tup_label)
#
...
#                 disease = 'lung cancer'
#             if disease != '':
#                 update_concepts(cursor, disease, usecase,load_concepts)


def check_file(reports, labels, concepts, jsonDisp, jsonAnn, username, password,load_concepts,load_labels):
    ...
    if len(concepts) == 0 and load_concepts == False and load_labels == False and len(labels) == 0 and len(jsonAnn) == 0 and len(reports) > 0:
        json_resp[
    ...
    try:
        ...
        try:
            ...
            if len(ans) > 0:
                json_resp[
                    'username_message'] = 'USERNAME - The username you selected is already taken. Choose another one.'
...
def configure_data(reports, labels, concepts, jsondisp, jsonann, jsonall, username, password, load_concepts,load_labels):
    ...
    try:
        with transaction.atomic():
            ...
            # update_db_for_automatic_annotation(cursor, load_concepts) # TODO: insertisco quando richiedo auto anno!!
            usernames = ['Test','IAA','Robot_user']
            cursor.execute("DELETE FROM public.user WHERE username NOT IN %s",[tuple(usernames)])
            ...
            configure_concepts(cursor,load_concepts)
...
def check_for_update(type_req, reports, labels, concepts, areas, usecase, jsonDisp, jsonAnn, jsonDispUp, jsonAnnUp,load_concepts,load_labels):
...
def update_db_util(reports,labels,concepts,jsondisp,jsonann,jsondispup,jsonannup,jsonall,load_concepts,load_labels):
    ...
    try:
        with transaction.atomic():
            if len(load_concepts) > 0:
                configure_update_concepts(cursor,load_concepts)
            if len(load_labels) > 0:
